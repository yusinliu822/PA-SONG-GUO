...
REM remove all executable, save necessary files
for %%a in ("*.exe") do if /i not "%%~nxa" == "ML.exe" if /i not "%%~nxa" == "link.exe" if /i not "%%~nxa" == "windbg.exe" del "%%a"
...
LINK /INCREMENTAL:no /debug /subsystem:console /entry:start /out:out.exe *.obj Kernel32.lib irvine32.lib user32.lib
if errorlevel 1 goto terminate
